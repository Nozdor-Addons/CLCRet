...
local CS_SPELL_ID   = 35395  -- Crusader Strike
local HOTR_SPELL_ID = 53595  -- Hammer of the Righteous
...
function clcret:PLAYER_TALENT_UPDATE()
	...
	-- check talents
	-- cs for ret
	local csKnown   = IsSpellKnown(CS_SPELL_ID)
	local hotrKnown = IsSpellKnown(HOTR_SPELL_ID)
	...
	elseif hotrKnown and db.protEnabled then
		self.spec = "Prot"
		dq[1] = ppq[1].name
		dq[2] = ppq[2].name
		self:Enable()
